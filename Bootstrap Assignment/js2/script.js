...
function fillBooks(){
	for(i=0;i<movies.length;i++){
		...
		<div class="col">
            <div class="card">
              <img class="card-img-top" src="`+movies[i].image+`">
              <div class="card-body">
                <h4 class="card-title" id="movie-title">`+movies[i].name+`</h4>
                <p class="card-text"><span id="movie-summary">`+movies[i].summary+`</span><br><span id="movie-category">`+movies[i].category+`</span></p>
                <a href="" class="btn btn-primary addToCart" id="book`+movies[i].movieid+`" data-toggle="popover" data-trigger="hover" data-placement="right"  data-html="true" data-content="<strong>Price: `+movies[i].price+`<br>GST: `+Math.round(movies[i].price * 1.1)+`</strong>">Add to Cart <i class="fa fa-cart-plus"></i></a>
              </div>
            </div>
          </div>
		`;
		$('#'+moviess[i].category+'>.container>.row').html( $('#'+movies[i].category+'>.container>.row').html()+bookCard );
	}
...
function add_category(){
	...
	var movieNames=[];
	$('#new_movies_add').children().map(function(index,moviename){
		bookCard = `
		<div class="col">
            <div class="card">
